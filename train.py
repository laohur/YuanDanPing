...
and https://github.com/shibing624/MedicalGPT/blob/main/pretraining.py
"""
import json
import math
import os
...
class ModelArguments:
    ...
    checkpointing: Optional[str] = field(default=True, metadata={"help": "Whether to load the model in 8bit mode or not."})
...
def find_all_linear_names(peft_model, int4=False, int8=False):
    ...
    logger.info(f"lora_module_names:{lora_module_names}")
...
def main():
    ...
    #     if x in model_args.model_name_or_path.lower():
    #         model_type=x
    config_class, model_class, tokenizer_class = MODEL_CLASSES["auto"]

    if model_args.model_type!="auto":
        Conf=json.load(open(f"{model_args.model_name_or_path}/config.json"))
        model_type="auto"
        for x in MODEL_CLASSES:
            if x in Conf["architectures"][0].lower():
                model_type=x    
        config_class, model_class = MODEL_CLASSES[model_type][:2]
    ...
    if model_args.model_type!="auto":
        Conf=json.load(open(f"{tokenizer_name_or_path}/tokenizer_config.json"))
        model_type="auto"
        for x in MODEL_CLASSES:
            if x in Conf["tokenizer_class"].lower():
                model_type=x    
        tokenizer_class = MODEL_CLASSES[model_type][-1]

    tokenizer = tokenizer_class.from_pretrained(tokenizer_name_or_path, **tokenizer_kwargs)
    tokenizer_index_map=[i for i in range(tokenizer.vocab_size)]
    random.shuffle(tokenizer_index_map)
    ...
    if data_args.dataset_name is not None:
        ...
        if data_args.train_file_dir is not None :
            train_data_files = glob(rf'{data_args.train_file_dir}', recursive=True)
        ...
        if data_args.validation_file_dir is not None:
            eval_data_files = glob(rf'{data_args.validation_file_dir}', recursive=True)
    ...
    if training_args.do_train:
        ...
        logger.debug(tokenizer.decode(train_dataset[0]['input_ids'][:100]))
    ...
    if training_args.do_eval:
        ...
        logger.debug(tokenizer.decode(eval_dataset[0]['input_ids'][:100]))
    ...
    if script_args.use_peft:
        ...
        # model = model.float()
    all_param,trainable_params=print_trainable_parameters(model)

    # Initialize our Trainer
    if model_args.checkpointing=="auto":
       training_args.gradient_checkpointing= all_param>5*1024**3
    else:
       training_args.gradient_checkpointing=bool(training_args.checkpointing) 
